...
private:
  ...
  std::unique_ptr<ExprAST> parsePrimary() {
    ...
    default:
      ...
      return nullptr;
    case tok_identifier:
      return parseIdentifierExpr();//自定义函数和变量
    case tok_number:
      return parseNumberExpr();//一个数字
    case '(':
      return parseParenExpr();//括号表达式
    case '[':
      return parseTensorLiteralExpr();//张量
    case ';':
      return nullptr;
    case '}':
      return nullptr;
  ...
  std::unique_ptr<ExprAST> parseBinOpRHS(int exprPrec,
    ...
    while (true) {
      ...
      if (tokPrec < nextPrec) {
        rhs = parseBinOpRHS(tokPrec + 1, std::move(rhs));//递归处理右侧表达式，递归调用自己右侧变左侧
        if (!rhs)
          return nullptr;
      }
  ...
  int getTokPrecedence() {
    if (!isascii(lexer.getCurToken()))//字符集判断
      return -1;
